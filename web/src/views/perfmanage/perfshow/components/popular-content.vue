<template>
  <a-spin :loading="loading" style="width: 100%">
    <a-card class="general-card" :header-style="{ paddingBottom: '0' }" :body-style="{ padding: '17px 20px 21px 20px' }">
      <template #title>
        {{ $t('perfshow.popularContent') }}
      </template>
      <template #extra>
        <a-link>{{ $t('perfshow.viewMore') }}</a-link>
      </template>
      <a-space direction="vertical" :size="10" fill>
        <a-radio-group v-model:model-value="type" type="button" @change="typeChange as any">
          <a-radio value="text">
            {{ $t('perfshow.popularContent.text') }}
          </a-radio>
          <a-radio value="image">
            {{ $t('perfshow.popularContent.image') }}
          </a-radio>
          <a-radio value="video">
            {{ $t('perfshow.popularContent.video') }}
          </a-radio>
        </a-radio-group>
...
import { queryPopularList } from '@/api/perfmanage';
